; Clojure indentation follows similar principles to Scheme, with special handling
; for def-like forms and let-like binding forms.
...
(list_lit . (_) @first . (_) @anchor
  ...
  (#not-match? @first "^(def|defn|defn-|defmacro|defmethod|defmulti|defonce|defprotocol|deftype|defrecord|defstruct|definline|definterface|ns|let|letfn|binding|loop|for|doseq|dotimes|when-let|if-let|when-some|if-some|with-.*|fn)$")) @align
...
; indent the list body by one level (assumes indent width of 2)
(list_lit . (_) @first .
  (#not-kind-eq? @first "bool_lit")
  (#not-kind-eq? @first "nil_lit")
  (#not-kind-eq? @first "str_lit")
  (#not-kind-eq? @first "num_lit")
  (#not-kind-eq? @first "kwd_lit")
  (#not-match? @first "^(def|defn|defn-|defmacro|defmethod|defmulti|defonce|defprotocol|deftype|defrecord|defstruct|definline|definterface|ns|let|letfn|binding|loop|for|doseq|dotimes|when-let|if-let|when-some|if-some|with-.*|fn)$")) @indent
...
(list_lit . (sym_lit) @first
  (#match? @first "^(def|defn|defn-|defmacro|defmethod|defmulti|defonce|defprotocol|deftype|defrecord|defstruct|definline|definterface|ns|let|letfn|binding|loop|for|doseq|dotimes|when-let|if-let|when-some|if-some|with-.*|fn)$")) @indent
